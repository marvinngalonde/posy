...
export default function PurchaseReturnsReport() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/purchases-return?id=${selectedReturn.id}`, {
        method: 'DELETE'
      })
  ...
  // Open view modal
  const openViewModal = (returnItem: PurchaseReturn) => {
    setSelectedReturn(returnItem)
    setShowViewModal(true)
  }

  // Open edit modal
  const openEditModal = (returnItem: PurchaseReturn) => {
    setSelectedReturn(returnItem)
    setFormData({
  ...
  // Open delete modal
  const openDeleteModal = (returnItem: PurchaseReturn) => {
    setSelectedReturn(returnItem)
  ...
  const handleExportPDF = () => {
    ...
    doc.text('Purchase Returns Report', 14, 16)
  ...
  const handleExportExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredReturns.map(returnItem => ({
        ...
        Reference: returnItem.reference,
        Supplier: returnItem.supplier_name || "-",
      ...
      <div className="p-6">
        <div className="mb-6">
          <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
            <span>Reports</span>
            <span>|</span>
            <span>Purchase Returns Report</span>
          </div>
          <h1 className="text-2xl font-bold">Purchase Returns Report</h1>
        </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={10} className="text-center p-6">No purchase returns found.</td>
                  </tr>
                ) : (
                  filteredReturns.map((returnItem) => (
                    <tr key={returnItem.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-3">{returnItem.supplier_name || "-"}</td>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="p-4">
                <p className="text-sm text-gray-600 mb-4">
                  Are you sure you want to delete purchase return "{selectedReturn.reference}"? This action cannot be undone.
                </p>
